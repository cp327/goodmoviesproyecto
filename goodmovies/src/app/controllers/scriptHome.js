...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Usar el nombre del archivo como categoría
  var categoria = nombreArchivo.replace('.php', '');

  // Concatenar la ruta con el nombre del archivo PHP
  var rutaCompleta = '../models/generar_descripcion.php' + (categoria ? '?categoria=' + categoria : '');

  // Obtener automáticamente las descripciones de la categoría
  fetch(rutaCompleta)
    ...
    .then(data => {
      ...
      var cambioTitulo = (index) => {
        ...
        if (descripcion && descripcion[index]) {
          ...
          console.log(`data-idpelicula actualizado a ${descripcion[index].idPelicula}`);
        } else {
...
//   description.addEventListener("wheel", function (event) {
//     // Ajusta el valor según la cantidad que deseas que se desplace
//     var scrollAmount = 1;

//     // Verifica la dirección del scroll (arriba o abajo)
//     if (event.deltaY > 0) {
