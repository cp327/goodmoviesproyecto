...
$sql = "SELECT * FROM favoritos WHERE IDuSUARIO = ? AND IDpELICULA = ?";
...
if ($stmt) {
    ...
    if ($stmt->num_rows > 0) {
        $response = array('status' => 'success', 'message' => 'Película ya está en tu lista de favoritos');
    } else {
        $response = array('status' => 'error', 'message' => 'Película no está en tu lista de favoritos');
    }
