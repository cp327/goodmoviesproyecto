...
$sql = "DELETE FROM descargas WHERE IDuSUARIO = ? AND IDpELICULA = ?";
...
if ($stmt) {
    ...
    if ($stmt->affected_rows > 0) {
        $response = array('status' => 'success', 'message' => 'Película eliminada de mis descargas correctamente', 'redirect' => '../views/misDescargas.php');

        
    } else {
        $response = array('status' => 'error', 'message' => 'Error al eliminar la película de mis descargas');
    }
